...
from insta_tg_sync.apify_sync import ApifySyncRunner
...
def test_dry_run_does_not_mark_new_apify_post_processed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("insta_tg_sync.apify_sync.download_apify_media", lambda *args: [media])

    runner = ApifySyncRunner(_config(tmp_path, dry_run=True))
    runner.telegram = FakeTelegram()
    runner.state.load()
    runner.state.mark_processed("account", "OLD")

    runner._process_post(_account(), _post())
...
def test_real_run_marks_apify_post_after_successful_send(tmp_path, monkeypatch):
    ...
    runner = ApifySyncRunner(_config(tmp_path))
    runner.telegram = FakeTelegram()
    runner.state.load()
    runner.state.mark_processed("account", "OLD")
...
def test_apify_usage_counts_requested_results_not_normalized_posts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("insta_tg_sync.apify_sync.ApifyInstagramClient", FakeApifyClient)
...
def test_initialize_only_marks_all_fetched_apify_posts_without_sending(tmp_path):
    runner = ApifySyncRunner(_config(tmp_path, initialize_only=True))
    runner.telegram = FakeTelegram()
    runner.state.load()
    runner.state.mark_processed("account", "PINNED")

    runner._sync_posts(_account(), [_post("PINNED"), _post("CURRENT"), _post("OLDER")])
